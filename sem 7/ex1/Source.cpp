...
public:
	...
	Student(int idMatricol, GEN gen, int nrNote, double* note,
		const char* nume, int varsta, string telefon)
		:ID_MATRICOL(
			idMatricol > 0 ? idMatricol : throw new exception("id matricol trebuie sa fie mai mare ca 0")
		) {
		...
		if (note == nullptr) {
			throw new exception("inputul note este gresit");
		}
	...
	void adaugaNoteInVector(double nota) {
		...
		delete[] note;//evita memory leaks
	...
	void stergeToateAparitiileNoteiDateDinVector(int nota) {
		int k = 0;
		for (int i = 0; i < nrNote; i++) {
			if (note[i] == nota) {
				k++;
		...
		if (k > 0) {
			double* paux = new double[nrNote - k];
	...
	void stergeONotaDeLaUnAnumitIndexDinVector(int index) {
		if (index >= 0 && index < nrNote) {
			double* paux = new double[nrNote - 1];
			...
			throw new exception("Eroare stergeONotaDeLaUnAnumitIndexDinVector(): index < 0 sau index > nrNote");
		}
	}
};
